...
func showAllWorkers() {
	...
	args := []string{"get", "pods", "-n", ns, "-o", "yaml"}
	...
	if len(podList.Items) == 0 {
		fmt.Println("Nenhum pod encontrado no namespace:", ns)
		return
	}
	...
	for _, pod := range podList.Items {
		...
		running, fatal, stopped := showWorkersForPod(pod.Metadata.Name, ns)
...
func showWorkersForPod(podName string, ns string) (int, int, int) {
	var args []string
	if ns == "" {
		args = []string{"exec", podName, "--", "supervisorctl", "status"}
	} else {
		args = []string{"exec", podName, "-n", ns, "--", "supervisorctl", "status"}
	}
