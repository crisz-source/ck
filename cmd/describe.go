...
func describePod(podName string) {
	ns := viper.GetString("namespace")    // ← TROCA Namespace por ns

	var args []string
	if ns == "" {
		args = []string{"get", "pod", podName, "-o", "yaml"}
	} else {
		args = []string{"get", "pod", podName, "-n", ns, "-o", "yaml"}
	}
	...
	fmt.Println("\n=== EVENTOS ===")
	getEvents(podName, ns)    // ← PASSA ns como parâmetro agora
}

// getEvents agora RECEBE ns ao invés de ler a global
func getEvents(podName string, ns string) {
	var args []string
	if ns == "" {
		args = []string{"get", "events", "--field-selector",
			fmt.Sprintf("involvedObject.name=%s", podName), "-o", "yaml"}
	} else {
		args = []string{"get", "events", "-n", ns, "--field-selector",
			fmt.Sprintf("involvedObject.name=%s", podName), "-o", "yaml"}
	}
